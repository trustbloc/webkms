...
Feature: KMS and crypto operations
  ...
  Scenario: User creates a key
    ...
    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys" to create "ED25519" key
    Then  user gets a response with HTTP status code "201 Created"
  ...
  Scenario: User signs a message and verifies a signature
    ...
    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys/{keyID}/sign" to sign "test message"
    Then  user gets a response with HTTP status code "200 OK"
     And  user gets a response with non-empty "signature"

    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys/{keyID}/verify" to verify "signature" for "test message"
    Then  user gets a response with HTTP status code "200 OK"
  ...
  Scenario: User encrypts/decrypts a message
    ...
    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys/{keyID}/encrypt" to encrypt "test message"
    Then  user gets a response with HTTP status code "200 OK"
     And  user gets a response with non-empty "cipherText"
     And  user gets a response with non-empty "nonce"

    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys/{keyID}/decrypt" to decrypt "cipherText"
    Then  user gets a response with HTTP status code "200 OK"
  ...
  Scenario: User computes/verifies MAC for data
    ...
    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys/{keyID}/computemac" to compute MAC for "test data"
    Then  user gets a response with HTTP status code "200 OK"
     And  user gets a response with non-empty "mac"

    When  user sends an HTTP POST to "https://localhost:8076/kms/keystores/{keystoreID}/keys/{keyID}/verifymac" to verify MAC "mac" for "test data"
    Then  user gets a response with HTTP status code "200 OK"
     And  user gets a response with no "errMsg"
