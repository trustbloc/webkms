...
The `kms-server` can be built from within the `cmd/kms-server` directory with `go build`.

## Run the server

Start the server with `./kms-server start [flags]`.
...
$ cd cmd/kms-server
$ go build
$ ./kms-server start --host-url localhost:8076 --database-type couchdb --database-url admin:password@couchdb.example.com:5984 --database-prefix kms \
